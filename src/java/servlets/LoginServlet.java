...
public class LoginServlet extends HttpServlet{
      ...
      try{ 
           ...
           if(boo){
              ...
              if(m==true){ 
                  ...
                  String tip="您输入的用户名不存在，或密码不般配";
                  //调用登录失败的方法:
                  fail(request,response,logname,tip); 
              }
           }
           else{
   ...
   public void success(HttpServletRequest request,HttpServletResponse response
      ...
      try{  log=(Log)session.getAttribute("log");
            if(log==null){
               log=new Log();  //创建新的数据模型 
               session.setAttribute("log",log);
               log=(Log)session.getAttribute("log");
            }
            String name =log.getLogname();
            if(name.equals(logname)) {
               log.setTip(logname+"已经登录了");
               log.setLogname(logname);
            }
            else {  //数据模型存储新的登录用户
                log.setTip(logname+"登录成功");
                log.setLogname(logname);
            }
      }
      catch(Exception ee){
    ...
    public void fail(HttpServletRequest request,HttpServletResponse response
                      ,String logname,String tip) {
        response.setContentType("text/html;charset=GB2312");
        try {
         PrintWriter out=response.getWriter();
         out.println("<html><body>");
         out.println("<h2>"+logname+"登录反馈结果<br>"+tip+"</h2>") ;
